...
import WeaponsGroup from "../weapons/weaponsGroup.js";
  export default class Level extends Phaser.Scene {
    ...
    deathZones = [];
    walls = [];
    fireRate = 135; // Tiempo de recarga en ms
  lastFired = 0;  // Tiempo del último disparo
    /**
    ...
    create() {
      ...
      this.input.on(
        "pointerdown",
        function () {
          const currentTime = this.time.now; // Obtener el tiempo actual
          if (!this.player.isDashing && currentTime - this.lastFired > this.fireRate) {
            this.player.isShooting = true;
            this.lastFired = currentTime; // Actualizar el tiempo del último disparo
            this.time.addEvent({
              delay: 200, // in ms
              callback: () => {
    ...
    hitEnemy(bullets, enemy) {
      ...
      if (enemy.life <= 0) {
        ...
        enemy.dead(this)
        let random = Math.floor(Math.random() * 30) + 1; //Probabilidad de aparecer consumible
        if(random === 1){
